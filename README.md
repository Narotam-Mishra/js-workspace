...
### [Implement your own `forEach()` method in JS / Write polyfill for `forEach()` method in JS.](https://github.com/Narotam-Mishra/js-workspace?tab=readme-ov-file#tricky-js-interview-coding-questions)

### Intuition
`forEach()` is a higher-order function that executes a callback function for each element in an array. It doesn't return anything (undefined) and is used for side effects like logging, DOM manipulation, etc.

### Approach
1. Validate inputs (callback must be a function)
2. Handle edge cases (sparse arrays, this context)
3. Iterate through array and call callback with proper parameters
4. Ensure proper this binding for the callback

### **Key Behaviors to Implement**
1. **Expecting a return value** :- `forEach` always returns undefined
2. **Breaking early** :- No break or return to exit early (use for loop instead)
3. **this binding** :- Remember to pass thisArg when needed
...
Index 4: 5
  ...
  2 * 10 = 20
  3 * 10 = 30
  Results after forEach: [ 'Item 1: APPLE', 'Item 2: BANANA', 'Item 3: CHERRY' ]
